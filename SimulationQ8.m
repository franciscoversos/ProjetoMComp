%%-------------------------------------------------------------------------
% Flow problem simulation using quadratic quadrilateral (Q8) elements
%%-------------------------------------------------------------------------
% Import the Nx files containing the nodes and elements, plus another file
% with results for comparison
format long
clear all
fileID0 = fopen('geral_quad8_nodes.txt', 'r');
fileID1 = fopen('geral_quad8_elements.txt', 'r');
Q8NxPotencial  = xlsread("Q8_malhabase_potencial.xlsx");        % File with Nx potential data
Q8NxVelocidade = xlsread("Q8_malhabase_velocidade.xlsx");      % File with Nx velocity data
formatSpec = '%c';'%d' ;
%%-------------------------------------------------------------------------
% Extract node coordinates, stored in the vector "coord"
...
%   fronteira1 = left wall
%   fronteira2 = top wall
%   fronteira3 = right wall
%   fronteira4 = bottom wall
v = 2500;
fronteira1=[];
fronteira2=[];
fronteira3=[];
...
% Neumann conditions
% According to the nodal forces, the forces are distributed as follows
nEl=0;
n=0;
k=0;
for l =1:1:N
...
%%-------------------------------------------------------------------------
% After exporting the Nx potential file, compute the error between the two results
EPotencial=[];
...
%%-------------------------------------------------------------------------
% Compute gradients at points of interest (SfluxuQuiver) and at centroids (Sfluxu),
% and also compute pressure
L=0;
Sfluxu=[];
Xpint=[];
SfluxuQuiver=[];
for i=1:length(Elements)
    ...
    csi=0;
    eta=0;
    nip = 4 ;    % reduced integration points
    [xp wp]=Genip2DQ (nip);
    ...
    for ip=1:nip
        ...
        fluxu = -gradu;
        Xpint=[Xpint, xpint];
        SfluxuQuiver = [SfluxuQuiver,fluxu]; % Flux at points of interest (gives a better map)
    end

    Sfluxu = [Sfluxu,fluxu]; % Flux at centroids
    pressao(i)=(0.5).*(2500^2 -(Sfluxu(1,i).^2)+ (Sfluxu(2,i).^2)); % Pressure at centroids
end

%%-------------------------------------------------------------------------
% Compute velocities at points of interest
...
El=[];
U=[];
% Compute nodal velocities by averaging element values
...
% Compute velocity error between MATLAB and Nx
for i =1:length(coord)
...
% Since an element is bounded by its two vertices, exclude interior nodes (even indices)
fronteira4d=[];
...
% Resultant force on the whole part
Fx =(F1x +F2x +F3x+F4x)*10^-6;
Fy =(F1y + F2y +F3y + F4y)*10^-6;
%%-------------------------------------------------------------------------
% Menu that allows the user to choose which plot to display
while true
    % Menu
    msg  = 'Choose the plot:';
    ...
    switch choice
        ...
        case 2 % Potential
            ...
            for i=1:length(Elements)
                ...
                no7= Elements(i,8);
                no8= Elements(i,9);
                edofs=[no1 no5 no2 no6 no3 no7 no4 no8]; % drawing order
        ...
        case 4 % Potential error (Nx vs MATLAB)
            figure;
            for i=1:length(Elements)
                no1= Elements(i,2);
                no2= Elements(i,3);
        ...
        case 5 % Velocity error (Nx vs MATLAB)
            figure;
            for i=1:length(Elements)
                no1= Elements(i,2);
                no2= Elements(i,3);
